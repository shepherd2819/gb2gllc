...
import { metaInstallUrl } from "@/lib/meta";
...
export async function GET(req: NextRequest) {
  ...
  if (requestedClientId && user.email === ADMIN_EMAIL) {
    ...
    if (!client) return NextResponse.redirect(`${ADMIN_URL}/clients?meta_install=client_not_found`);
    clientId = client.id;
    mode = "admin";
  } else {
  ...
  const url = metaInstallUrl(state);

  const res = NextResponse.redirect(url);
  res.cookies.set("meta_install_state", state, {
    httpOnly: true,
    secure: true,
    sameSite: "lax",
    maxAge: 600,
