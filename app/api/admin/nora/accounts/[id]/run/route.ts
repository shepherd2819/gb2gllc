...
export async function POST(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  // Refresh metrics
  const { data: acct } = await supabaseAdmin.from("nora_accounts").select("stripe_secret_key").eq("id", id).single();
  if (acct?.stripe_secret_key) {
    try {
      const m = await fetchMetrics(acct.stripe_secret_key);
