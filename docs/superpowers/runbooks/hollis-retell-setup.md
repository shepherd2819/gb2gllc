...
1. In Retell, **duplicate the template agent** → name it "GB2G Demo". This agent is **GB2G's own AI concierge**: it answers questions about GB2G and explains what an AI agent could do for the caller's business (not a fictional business). The route injects GB2G-aware demo dynamic variables (`business_name: "GB2G"`, plus `services`/`faq`/`greeting` all about GB2G), so the prompt can use `{{business_name}}`, `{{services}}`, `{{faq}}`, `{{greeting}}` exactly like the production agent — no separate prompt authoring needed. Set a short **`max_call_duration_ms`** (e.g. 180000 = 3 min) to cap cost. (Tools like booking are optional on the demo agent; the concierge mainly explains + invites a follow-up via the page's contact CTA.)
