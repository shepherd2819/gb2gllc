...
const CATEGORIES = ["bug", "feature_request", "account_help", "billing_question", "general", "spam"];
const STATUSES   = ["classified", "new", "sent", "archived", "flagged", "all"];

const CAT_COLOR: Record<string, string> = {
  bug:              "#8a4a4a",
  feature_request:  "#2c7a4a",
  account_help:     "#7a4c2c",
  billing_question: "#4a4a8a",
  general:          "#5a5a5a",
  spam:             "#8a8a4a",
};
...
export function WrenInbox({ account, initialMessages, settings, currentStatus, currentCategory }: {
  account: Account;
  ...
  function setFilter(key: "status" | "category", value: string) {
    ...
    window.location.href = `/agents/wren/${account.id}${qs ? `?${qs}` : ""}`;
  }

  async function pollNow() {
    setPollBusy(true);
    try {
      const res = await fetch(`/api/admin/wren/accounts/${account.id}/poll`, { method: "POST" });
      ...
      <div className="admin-page-header">
        <div>
          ...
          <span className="client-email">
            ...
            {" · "}<a href="/agents/wren" className="at-link">← All inboxes</a>
          </span>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <button className="admin-btn admin-btn-sm" onClick={pollNow} disabled={pollBusy}>
...
function MessageDetail({ message, onUpdate }: { message: Message; onUpdate: (p: Partial<Message>) => void }) {
  ...
  const [busy, setBusy] = useState<"" | "save" | "send" | "archive" | "redraft" | "flag">("");
  ...
  async function flagMessage() {
    ...
    setBusy("");
    if (res.ok) onUpdate({ status: "flagged" });
  }
    ...
    <div className="admin-card" style={{ padding: 20, maxHeight: "75vh", overflowY: "auto" }}>
        ...
        <div style={{ marginBottom: 14, padding: 12, background: "rgba(28,30,27,0.03)", borderRadius: 4, fontSize: 13 }}>
          {message.suggested_action && (
            <div style={{ marginBottom: 4 }}>
              <span style={{ fontSize: 10, textTransform: "uppercase", letterSpacing: "0.06em", color: "var(--text-mute)" }}>Wren suggests · </span>
      ...
      <div>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          <button className="admin-btn-ghost admin-btn-sm" onClick={redraft} disabled={busy !== ""}>
          ...
          <button className="admin-btn-ghost admin-btn-sm" onClick={flagMessage} disabled={busy !== "" || message.status === "flagged"}>
            {busy === "flag" ? "…" : "Flag"}
          </button>
        </div>
      </div>
...
function SettingsPanel({ accountId, initial, onClose }: { accountId: string; initial: Settings; onClose: () => void }) {
  ...
  async function save() {
    setBusy(true);
    const res = await fetch(`/api/admin/wren/accounts/${accountId}/settings`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  return (
    <div className="admin-card" style={{ padding: 18, marginBottom: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 14 }}>
        <h3 style={{ margin: 0, fontSize: 14, fontWeight: 500 }}>Wren settings</h3>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <textarea
          ...
          rows={3}
          className="admin-input"
          placeholder="e.g. Empathetic and clear. No jargon. Sign off as 'Support Team'. Acknowledge the issue before offering a solution."
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        ...
        <span style={{ fontSize: 11, color: "var(--text-mute)", marginLeft: "auto" }}>
          Changes apply to future polls. To re-run Wren on existing messages, open each and click Re-draft.
        </span>
      </div>
    </div>
  );
